...
JRESULT TJpg_Decoder::drawFsJpg(int32_t x, int32_t y, fs::File inFile) {
  ...
  jresult = _jd_prepare(&jdec, jd_input, workspace, TJPGD_WORKSPACE_SIZE, (unsigned int)0);

  // Extract image and render
  if (jresult == JDR_OK) {
    jresult = _jd_decomp(&jdec, jd_output, jpgScale);
  }
...
JRESULT TJpg_Decoder::getFsJpgSize(uint16_t *w, uint16_t *h, fs::File inFile) {
  ...
  *h = 0;
  ...
  jresult = _jd_prepare(&jdec, jd_input, workspace, TJPGD_WORKSPACE_SIZE, 0);
